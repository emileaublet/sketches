...
const nodesSketch = (p: p5SVG) => {
  ...
  const drawShadedBall = () => {
    ...
    const drawCircle = (
      ...
      if (debug) {
        const colorIndex =
          p.floor(p.map(intensity, 0, 1, colors.length - 1, 0)) % colors.length;
    ...
    for (let y = -ballRadius; y <= ballRadius; y += gapFillSpacing) {
      for (let x = -ballRadius; x <= ballRadius; x += gapFillSpacing) {
        ...
        for (let attempt = 0; attempt < 5; attempt++) {
          const jitterX = p.random(-gapFillSpacing * 0.8, gapFillSpacing * 0.8);
          const jitterY = p.random(-gapFillSpacing * 0.8, gapFillSpacing * 0.8);
